...
void SF_IMU::update(){
    ...
    // 限制陀螺仪角度在-180到180度范围内（处理Z轴角度连续性）
    while (angleGyro[2] > 180.0) angleGyro[2] -= 360.0;
    while (angleGyro[2] < -180.0) angleGyro[2] += 360.0;

    //融合加速度和陀螺仪角度
    //使用加权平均（融合算法）将陀螺仪和加速度计的角度值融合在一起，以获得更精确和稳定的角度值。其中gyroCoef和accCoef是两个权重系数，表示对陀螺仪和加速度计数据的信任程度。
    
    // 处理加速度计角度的连续性（避免-180到+180的跳跃）
    float accAngle0 = angleAcc[0];
    float accAngle1 = angleAcc[1];
    
    // 检查X轴角度跳跃
    if (abs(angle[0] - accAngle0) > 180) {
        if (angle[0] > accAngle0) {
            accAngle0 += 360;
        } else {
            accAngle0 -= 360;
        }
    ...
    // 检查Y轴角度跳跃
    if (abs(angle[1] - accAngle1) > 180) {
        if (angle[1] > accAngle1) {
            accAngle1 += 360;
        } else {
            accAngle1 -= 360;
        }
    ...
    angle[0] = (_gyroCoef * (angle[0] + gyro[0] * _interval)) + (_accCoef * accAngle0);
    angle[1] = (_gyroCoef * (angle[1] + gyro[1] * _interval)) + (_accCoef * accAngle1);
    angle[2] = angleGyro[2];
